#source("/home/lotspesc/SMLE_LogReg/logreg_2ph.R")
library(logreg2ph)
...
for (N in c(1000))
{
  for (pv in c(0.25))
  {
    ...
    sim_res <- data.frame(sim = 1:num_sims, N, n, beta0, beta1, beta2, delta3, PhaseII = audit,
    ...
    naive_res <- complete_res <- tang_res <- smle_res <- sim_res
    ...
    for(s in 1:num_sims)
    {
      ...
      smle <- logreg2ph::logreg2ph(Y_unval = "Ystar", Y_val = "Y", X_unval = "Xstar", X_val = "X", C = "Z",
                                   Validated = "V", Bspline = colnames(B),
      ...
      write.csv(rbind(smle_res, tang_res, naive_res), paste0("delta", delta3, "_", N, "_pv", 100*pv,"_seed", sim_seed, "_", ifelse(audit == "SRS", "SRS", "CC"), ".csv"), row.names = FALSE)
    }
  }
}
