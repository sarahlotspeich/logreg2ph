...
library(logreg2ph)
setwd("~sarahlotspeich/Dropbox/Vanderbilt/Lotspeich_Dissertation/logreg_2ph/misspecYstar/")
...
smle_res <- data.frame(sim = 1:num_sims, N, n, beta0, beta1, beta2, theta5, PhaseII = audit,
                      ...
                      conv = NA, se_conv = NA, conv_msg = NA, method = "SMLE")
...
for(s in 1:num_sims)
{
  ...
  write.csv(smle_res, paste0("correctSMLE/correctSMLE_theta", theta5, "_", N, "_pv", 100*pv,"_seed", sim_seed, "_", ifelse(audit == "SRS", "SRS", "CC"), ".csv"), row.names = FALSE)
}
