logreg_2ph_Yonly <- function(N, n, Y_val=NULL, Y_unval=NULL, X_val=NULL, C=NULL, X_val_interact = NULL,
  ...
  if (!CONVERGED) {
    ...
    return (list(Coefficients = data.frame(Coefficient = rep(NA, nrow(prev_theta)), 
                                          SE = NA),
                h_n = NA,
                converged = CONVERGED,
  ...
  if (noSE) {
    ...
    single_pert_theta <- sapply(X = seq(1, ncol(I_theta)), FUN = pl_theta_Yonly, theta = new_theta, h_n = h_n, n = n, N = N, 
                                ...
                                comp_dat_all = comp_dat_all, gamma0 = new_gamma, TOL = TOL, MAX_ITER = MAX_ITER)
    
    if (!(TRUE %in% is.na(single_pert_theta))) {
      ...
      cov_theta <- matrix(NA, nrow = nrow(I_theta), ncol = ncol(I_theta))
    }
