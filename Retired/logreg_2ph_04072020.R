...
pl_theta <- function(k, theta, h_n, n_v, n, Y_unval, Y_val, X_unval, X_val, C, Bspline, 
...
logreg_2ph <- function(Y_unval = NULL, Y_val = NULL, X_unval = NULL, X_val = NULL, C = NULL, 
                       Validated = NULL, Bspline = NULL, data, initial_lr_params = "Zero",
                       h_n_scale = 1, noSE = FALSE, TOL = 1E-4, MAX_ITER = 1000, rescale = FALSE)
{
  n <- nrow(data)
  ...
  if(0 %in% colSums(data[c(1:n_v), Bspline])) {
    return(list(Coefficients = data.frame(Coefficient = NA, SE = NA),
                h_n = NA,
                converged = FALSE,
                se_converged = FALSE,
  ...
  # one row per x) --------------------------------------------------
  suppressWarnings(comp_dat_unval <- cbind(re_data[-c(1:n_v),c(Y_unval, X_unval, C, Bspline)], x_obs_stacked))
  ...
  comp_dat_unval <- data.matrix(cbind(rbind(comp_dat_y0, comp_dat_y1), k = rep(rep(seq(1, m), each = (n - n_v)), times = 2)))
  ...
  while(it < MAX_ITER & !CONVERGED) {
    ...
    ## Update {p_kj} --------------------------------------------------
    ### Update numerators by summing u_t over i = 1, ..., n -----------
    new_p_num <- p_val_num + rowsum(u_t, group = rep(seq(1, m), each = (n - n_v)), reorder = TRUE)
    new_p <- t(t(new_p_num) / colSums(new_p_num))
    #if(VERBOSE) print(new_p[1,])
    ### Check for convergence -----------------------------------------
    p_conv <- abs(new_p - prev_p) < TOL
  ...
  if(!CONVERGED) {
    ...
    return(list(Coefficients = data.frame(Coefficient = new_theta, SE = NA),
                h_n = NA,
                converged = CONVERGED,
                converged_msg = CONVERGED_MSG,
                gamma_at_conv = matrix(NA, nrow = nrow(new_gamma)), 
  ...
  if(noSE & CONVERGED) {
    ...
    return(list(Coefficients = data.frame(Coefficient = re_theta, SE = NA),
                h_n = NA,
                converged = CONVERGED,
                converged_msg = CONVERGED_MSG,
