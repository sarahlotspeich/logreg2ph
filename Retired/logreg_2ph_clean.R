profile_out <- function(theta, n_v, n, Y_unval = NULL, Y_val = NULL, X_unval = NULL, X_val = NULL, C = NULL, Bspline = NULL, comp_dat_all, gamma0, p0, p_val_num, TOL, MAX_ITER) {
  sn <- ncol(p0)
  m <- nrow(p0)
...
observed_data_loglik <- function(n, n_v, Y_unval = NULL, Y_val = NULL, X_unval = NULL, X_val = NULL, C = NULL, 
...
pl_theta <- function(k, theta, h_n, n_v, n, Y_unval, Y_val, X_unval, X_val, C, 
                     Bspline, comp_dat_all, gamma0, p0, p_val_num, TOL, MAX_ITER) {
  pert <- theta
  pert[k] <- pert[k] + h_n
                            ...
                            rescale = FALSE)
{
  ...
  if(noSE) {
    ...
    single_pert_theta <- sapply(X = seq(1,ncol(I_theta)), 
                                FUN = pl_theta, 
                                theta = new_theta, h_n = h_n, n_v = n_v, n = n, 
