...
TwoPhase_LogReg <- function(Y_unval=NULL, Y_val=NULL, X_unval=NULL, X_val=NULL, C=NULL, Validated = NULL, Bspline=NULL, data, initial_lr_params = "Zero", h_n_scale = 1, rescale_if = 0.1, rescale_by = 0.9, h_n_adapt = FALSE, newton_step_scale = 1, noSE=FALSE, VERBOSE = FALSE, TOL_theta = 1E-6, TOL_gamma = 1E-4, TOL_p = 1E-4, MAX_ITER = 1000)
{
  n <- nrow(data)
  ...
  if(TRUE %in% abs(cd_params[-1]/cd_params[1]) < rescale_if)
  {
    for(p in which(abs(cd_params[-1]/cd_params[1]) < rescale_if))
    {
      rescale_param <- 1
      while(abs(cd_params[-1][p]/cd_params[1]) < rescale_if)
      {
        rescale_dat <- data.frame(data[c(1:n_v),])
        rescale_param <- rescale_param*rescale_by
  ...
  } else
  {
    ...
    S_theta <- matrix(0, nrow = nrow(new_theta), ncol = 1)
    for (k in 1:ncol(I_theta))
    {
      ...
      S_theta[k] <- od_loglik_pert_k - od_loglik_theta
